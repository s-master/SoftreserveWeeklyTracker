...
- Groups **without `softresID`** import loot only (no `SoftReserve` rows)
...
Available when `ASPNETCORE_ENVIRONMENT=Development` or `Debug:Enabled=true` in config.
...
| Item overview | Yes | All `(player, item)` balances for roster; includes “received” flag |
| Session / week / archive | Yes | Standard sort/search |

Tables use shared helper `initSoftresDataTable()` (`wwwroot/js/site.js`). Item names use Wowhead tooltips (`Views/Shared/_ItemLink.cshtml`); item column uses `white-space: nowrap` with horizontal scroll if needed.
...
| Item display | Item ID in HTML; Wowhead `tooltips.js?locale=3` (DE) or `locale=0` (EN) |
| Item links | `de.wowhead.com/tbc/item=…` or `www.wowhead.com/tbc/item=…` with `data-wowhead="item=…&domain=de\|www"` |
| Stored item names | **Not used** in views (Gargul `itemLink` is parsed but not displayed) |
